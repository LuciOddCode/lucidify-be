...
class GeminiService {
    ...
    async generateChatResponse(message, context = "", language = "en") {
        try {
            ...
            return {
                content: "I apologize, but I cannot provide a response at this time. Please try again later.",
                suggestions: [
    ...
    async generateJournalPrompt(mood, previousEntries) {
        try {
            ...
            const response = await result.response;
            const content = response.text();
            return (content || "How are you feeling today? What thoughts are on your mind?");
        }
        catch (error) {
    ...
    async generateCopingSuggestions(mood, emotions, language = "en") {
        try {
            const prompt = `Based on a mood rating of ${mood}/10 and emotions: ${emotions.join(", ")}, suggest 3-5 specific coping strategies. 
    ...
    async generateSessionSummary(steps, overallMood, language = "en") {
        try {
